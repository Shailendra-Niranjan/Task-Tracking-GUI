...
const EditTaskPopup = ({ setShowModal, taskToEdit }) => {
    ...
    useEffect(() => {
      if (taskToEdit) {
        setTaskDetails({
          ...
          startAt: taskToEdit.startAt || new Date().toISOString(),
          endAt: taskToEdit.endAt || "",
        });
      }
    }, [taskToEdit]);
    ...
    const handleSubmit = async () => {
      ...
      try {
        const response = await fetch(`/api/user/editTask/${taskToEdit.id}`, {
          method: "POST",
          headers: {
